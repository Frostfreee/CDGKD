...
from DGKD import DGKD, SaveBestStudentModelCallback
...
def distilling_train(args):
    ...
    teachers_paths = []
    teachers_paths.append(f"./ts_models/teacher/{args.data_type}/best_model_1e-04")
    teachers_paths.append(f'./ts_models/TA_1/{args.data_type}/DGKD_best_model_{learning_rate_str}')
    teachers_paths.append(f'./ts_models/TA_2/{args.data_type}/DGKD_best_model_{learning_rate_str}')
    ...
    # 确定模型的保存路径
    model_path = f'./ts_models/{student_str}/{args.data_type}/DGKD_best_model_{learning_rate_str}'
    ...
    # 要保存的CSV文件路径
    csv_file_path = './result_csv/self_csv.csv'
    # 检查CSV文件是否存在
    ...
    dist = DGKD(student_model=student_model, teacher_models=teacher_models, train_mode=args.train_mode)
    dist.compile(
    ...
    record_experiment(args, fittedModel, test_loss, test_acc, csv_file='./result_csv/DGKD_result.csv')
